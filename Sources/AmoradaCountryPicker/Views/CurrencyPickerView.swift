...
public struct CurrencyPickerView: View {
    ...
    public var body: some View {
        VStack(alignment: .leading){
            ...
            Button(action:{
                ...
                HStack{
                    Text(selectedCountry.map{ "\($0.emoji ?? "") \($0.currencyCode) - \($0.currencyName)" } ?? placeholderText)
                        .font(textStyle)
                    ...
                    LazyVStack(alignment: .leading,spacing: 8){
                        ForEach(filteredCountries(), id: \.countryCode){ country in
                            Button(action: {
                                ...
                                Text("\(country.emoji ?? "") \(country.currencyCode) - \(country.currencyName)")
                                    .font(textStyle)
                                    .padding(.horizontal)
                            }
    ...
    private func filteredCountries() -> [Country] {
        ...
        return countries.filter {
            ($0.currencyName?.localizedCaseInsensitiveContains(searchText) ?? false) ||
